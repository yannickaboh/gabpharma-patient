...
- **Carte réelle (écran 17, Suivi de livraison)** : toujours un fond illustratif dégradé + pins Material — pas un problème de SDK (le SDK Google Maps est intégré depuis le 23 août 2026, voir écran 9 ci-dessous), mais d'absence de donnée : aucune position temps réel du livreur n'existe côté backend (`propositions_backend.md`, section 4 étape 3, cadré mais non implémenté). Rien à changer côté app tant que cette donnée n'existe pas.
...
2. **Connexion à l'API mobile réelle** (Django) : en cours depuis le 13 août 2026, suivie module par module dans `branchement_patient.md`. **Tous les modules d'authentification et fonctionnels sont désormais branchés et vérifiés** (Authentification, Accueil, Recherche+catalogue, Détail médicament/pharmacie, Favoris, Panier, Checkout, Paiement simulé, Commandes, Assurance, Notifications, Support, Profil, Inscription, Mot de passe oublié). Il ne reste que le point 15 du fichier de suivi, **bloqué côté backend** : token FCM (notifications push) — `push_notification_service.dart` récupère déjà un token localement mais aucun endpoint Django n'existe pour l'enregistrer ; nécessite un développement backend avant de pouvoir reprendre côté app. Gap connu non corrigé : pas de rafraîchissement de token JWT (expire à 20 min, pas d'endpoint côté Django) — l'app déconnecte proprement au lieu de planter, mais l'utilisateur doit se reconnecter manuellement toutes les 20 min pendant les sessions de test. Gap découvert au module Inscription : `/mobile/auth/resend-2fa/` ne fonctionne pas pour un code d'inscription (filtré à `LOGIN_2FA` côté serveur) et une réinscription avec le même e-mail échoue même si le compte précédent est resté `pending` — un code d'inscription expiré (5 min) ou perdu bloque l'utilisateur sans recours dans l'app, voir `branchement_patient.md` point 13. Gap similaire mais contourné au module Mot de passe oublié : le même endpoint de renvoi ne fonctionnerait pas non plus pour un code de réinitialisation, mais ici un vrai renvoi existe (ré-appeler `/password-reset/`) — voir `branchement_patient.md` point 14 pour le piège du challenge factice en cas de renvoi trop rapide.
...
- Compte de démo pratique (mêmes identifiants que ceux pré-remplis par défaut sur l'écran de connexion, donc zéro saisie manuelle nécessaire) : `patient.demo@gabpharma.ga` / `Demo1987.`, créé via `manage.py shell` le 13 août. **Mot de passe changé le 23 août 2026** lors du test réel du changement de mot de passe (module Profil) — `demonstration` ne fonctionne plus, le pré-remplissage de l'écran de connexion (`auth_screens.dart`) a été mis à jour en conséquence.
